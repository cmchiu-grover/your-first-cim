...
from backend.app.services import common
from backend.app.db.crud import (
    update_standard_time_value,
    get_current_active_user,
    update_eqp_status_comment,
)
from backend.app.db.dbquery import query_eqp_data
from backend.app.models.update_temp_oee import update_temp_oee_after_updating_stdt


def _accumulate_temp_result(temp_result_list, affected_eqp_ids, affected_details):
    for item in temp_result_list:
        affected_eqp_ids.add(item.get("eqp_id"))
        affected_details.append(item)


def _build_oee_details_text(affected_details):
    details_text = ""
...
async def _notify_stdt_update(current_user, updated_count, affected_eqp_ids, details_text):
    await common.notify_users(
        current_user,
        title="標準工時更新",
        message=(
...
async def update_standard_times_batch(data: List[dict], request: Request):
    current_user = await get_current_active_user(request)
    common.require_position(current_user, "IE")
    ...
    for row in data:
        if not update_standard_time_value(row):
            raise AppError(400, "更新失敗，找不到符合條件的資料...")
        ...
        _accumulate_temp_result(temp_result_list, affected_eqp_ids, affected_details)
        updated_count += 1

    details_text = _build_oee_details_text(affected_details)
    print(f"affected_details: {affected_details}")

    await _notify_stdt_update(current_user, updated_count, affected_eqp_ids, details_text)
...
async def upload_standard_times_csv(request: Request, file: UploadFile):
    current_user = await get_current_active_user(request)
    common.require_position(current_user, "IE")

    if not file.filename.endswith(".csv"):
        raise AppError(400, "只支援 CSV 檔案。")
...
async def update_eqp_status_comments(data: List[dict], request: Request):
    current_user = await get_current_active_user(request)
    common.require_login(current_user)
